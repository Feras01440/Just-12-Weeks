...
5. **Prototype scaling is emulated** (a font-size multiplier). Production must bind to platform Dynamic Type / non-linear font scaling, where intermediate steps (135%, 160%) need the re-rank threshold chosen deliberately — recorded as an open production task in `design-refinement/KNOWN-RISKS.md`.
